    ...
    list_data() {
      ...
      this.data.filter(graph => this.text_categories.includes(graph.category.name)).forEach(graph => {
        records = records.concat(graph.values.map(record => {
          ...
          record.formatted_date = moment((record.timestamp + this.offset()) * 1000).format('hh:mm:ss DD.MM.YYYY')
          record.date = moment((record.timestamp + this.offset()) * 1000).format('DD.MM.YYYY')
          return record
        }))
      })
